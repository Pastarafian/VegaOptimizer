/**
 * VegaOptimizer — Full VegaMCP Audit Script (v2 — Plain Text Output)
 * 
 * Same analysis as before but outputs in plain ASCII to avoid encoding issues.
 */

import fs from 'fs';
import path from 'path';

const VEGA_CAP = './build/tools/capabilities';
...
log('');
log('  VEGAMCP FULL AUDIT - VegaOptimizer Codebase');
...
try {
  const el = pr(await handleDesktopTesting({ action: 'extract_logs' }));
  log(`  Logs: ${el.logs?.length || el.line_count || JSON.stringify(el).substring(0,80)}`);
} catch(e) { log(`  WARN: ${e.message}`); }
try {
...
try {
  const r = pr(await handleDatabaseTesting({ action: 'integrity_check' }));
  log(`  Integrity: ${JSON.stringify(r.result || r).substring(0,200)}`);
} catch(e) { log(`  WARN: ${e.message}`); }
...
try {
  const r = pr(await handleServerTesting({ action: 'tls_check' }));
  log(`  TLS Check: ${JSON.stringify(r.result || r).substring(0,200)}`);
} catch(e) { log(`  WARN: ${e.message}`); }
...
try {
  const r = pr(await handleSecurityTesting({ action: 'sast_audit' }));
  log(`  SAST: ${JSON.stringify(r.result || r).substring(0,200)}`);
} catch(e) { log(`  WARN: ${e.message}`); }
try {
...
if (mainApp) {
  try {
    const r = pr(await handleCodeAnalysis({ action: 'complexity_metrics', code: fs.readFileSync(mainApp,'utf-8'), filename: 'App.tsx' }));
    log(`  Complexity: ${JSON.stringify(r.metrics || r).substring(0,200)}`);
  } catch(e) { log(`  WARN: ${e.message}`); }
  try {
    const r = pr(await handleCodeAnalysis({ action: 'import_graph', code: fs.readFileSync(mainApp,'utf-8'), filename: 'App.tsx' }));
    log(`  Imports: ${r.imports?.length || r.graph?.length || JSON.stringify(r).substring(0,100)}`);
  } catch(e) { log(`  WARN: ${e.message}`); }
}
...
try {
  const r = pr(await handleVisualTesting({ action: 'baseline_compare' }));
  log(`  Baseline: ${JSON.stringify(r.result || r).substring(0,200)}`);
} catch(e) { log(`  WARN: ${e.message}`); }
